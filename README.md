...
Not included, because they are governed by their own licences, live only in the training environment, or are too large to release here:
...
- **The RadBERT-based 18-class labeller used to score CRG**, and the **LLM-judge script** for the paper's whole-report Judge column. Both are released with CT-CHAT (linked above); this repository only provides the CRG *formula* (`pipeline/11_score_crg.py`) that consumes their output.
- **The exact CT-RATE validation manifest (3,039 volumes) and the 1,564 patient-series case list used for Table 1's primary protocol**, and **the 32-sample-per-volume candidate pool** sampled from CT-CHAT for the paper. These exist only as intermediate artefacts inside the training environment used for this project and were not exported before this release was prepared. `pipeline/11_score_crg.py --dedup-series` reimplements the de-duplication rule exactly (keep the first reconstruction per patient-series, ordered `valid_{patient}_{series}_{reconstruction}`), so the same 1,564-case protocol can be reconstructed from any validation-split label CSV with that naming; regenerating the candidate pool itself follows the same seeded-but-not-bit-reproducible caveat as step 2 below. This is a known gap — see `reproduce/run_table1.sh` for exactly where it bites, and the note in this repository's pull request for the authors to close it before camera-ready if the venue expects it.
